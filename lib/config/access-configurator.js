...
export function changeUserRoles (user, roles) {
  console.error(`changeUserRoles user ${user} roles ${roles}`)
...
export function walkDirs (dir, filelist, onlyName) {
  return Array.from(walkSync(dir, filelist))
    .filter(item => item.indexOf('.md') === -1)
    .map(item => item.substring(item.indexOf('/pages/') + (onlyName ? 7 : 0)))
}
