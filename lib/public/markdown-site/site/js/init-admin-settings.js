...
$(document).ready(() => {
  ...
  $.post('manage-access/get-all-settings').done((data) => {
    ...
    let tb = $('#access-table-body')
    data.map.forEach(rule => {
      let users = ''
      let paths = ''

      rule.users.forEach(u => {
        users += `<div class="chip chip-user">${u}</div>`
      })

      rule.paths.forEach(p => {
        paths += `<div class="chip chip-path">${p}</div>`
      })

      let us = `<td style="width: 40%">${users}</td>`
      let ps = `<td style="width: 40%">${paths}</td>`
      let html = `<tr><td style="max-width: 20%; min-width: 20%; width: 20%; height: auto; word-break: break-word;">${rule.name}</td>${us}${ps}</tr>`
