...
export default () => {
  ...
  router.route('/').get(isLoggedInAndHaveAccess, (req, res) => {
    // markdownToHtml(`${appRoot}/views/pages/${globalConfig.main_pages_folder}/readme.md`, 'Markdown Site', req.user.roles, html => {
    //   res.send(html)
    // })
    res.redirect(`pages/${globalConfig.main_pages_folder}/readme`)
  })
