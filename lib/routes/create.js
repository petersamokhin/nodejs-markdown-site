...
import { changePathsManyRoles, getAllParentPaths, isLoggedInAndHaveAccess, walkDirs } from '../config/access-configurator'
...
export default () => {
  ...
  router.route('/create').post(isLoggedInAndHaveAccess, (req, res) => {
    Configuration.getSeveral((items) => {
      ...
      if (req.body.old_path) {
        ...
        fs.unlink(fp, (errFp) => {
          if (errFp) throw showThrowableError(res, errFp, 'Can not delete old file')
        })
      }
  ...
  router.route('/create').delete((req, res) => {
    fs.unlink(`${appRoot}/views${req.body.path}.md`.replace(/\/\//g, '/').replace(/\/pages\/pages/g, '/pages'), (errFs) => {
      Path.deleteByPath(req.body.path, (errDb) => {
        console.log('!errDb && !errFs: ' + (!errDb && !errFs))

        if (errDb || errFs) {
          console.log('errFs: ' + errFs)
          console.log('errDb: ' + errDb)
        }
        res.send({ok: !errDb && !errFs})
      })
    })
  })
